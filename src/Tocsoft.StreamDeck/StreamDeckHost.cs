...
namespace Tocsoft.StreamDeck
{
    internal class StreamDeckHost : IHostedService
    {
        ...
        private readonly Dictionary<string, ActionDefinition> actions;
        ...
        public StreamDeckHost(IStreamDeckConnection streamDeckConnection, StreamDeckConfiguration configuration, IServiceProvider serviceProvider, ILogger<StreamDeckHost> logger)
        {
            this.streamDeckConnection = streamDeckConnection;
            this.actions = configuration.Actions.ToDictionary(x => x.UUID, x => x, StringComparer.OrdinalIgnoreCase);
